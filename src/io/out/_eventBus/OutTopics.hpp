 ...
 *
 * @file OutTopics.hpp
 * @brief Domain topics for remote Out over ITransport.
 */
#pragma once
...
{
public:
    OutTopics(tools::design::ipc::TopicScheme topics, std::string objectName) : _topics(std::move(topics)), _object(std::move(objectName))
    {
    }

    [[nodiscard]] const std::string& objectName() const noexcept { return _object; }

    [[nodiscard]] std::string set() const { return channel("set"); }
...
private:
    ...
    {
        return _topics.appTopic({"out", _object, name});
    }
