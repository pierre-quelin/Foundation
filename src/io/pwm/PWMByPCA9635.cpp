...
PWMByPCA9635::PWMByPCA9635(const shared_ptr<PCA9635> device,
                           const PCA9635::Led pin) :
    IPWM(),
    _device(device),
    _pin(pin),
...
PWMByPCA9635::PWMByPCA9635(ApplicationServices& app, Node node) :
    PWMByPCA9635(obtain<PCA9635>(app, node, "PCA9635"), node.at("Led").as<PCA9635::Led>())
{
}

PWMByPCA9635::~PWMByPCA9635()
{
    (void)state(State::Off);
}

int PWMByPCA9635::init()
{
...
IPWM::DutyCycleCtrl PWMByPCA9635::dutyCycleCtrl() const
{
    const std::shared_lock<std::shared_mutex> lock(_mutex);
    return _dutyCycleCtrl;
}

int PWMByPCA9635::dutyCycleCtrl(const DutyCycleCtrl& ctrl)
{
...
IPWM::State PWMByPCA9635::state() const
{
    const std::shared_lock<std::shared_mutex> lock(_mutex);
    return _state;
}

int PWMByPCA9635::state(const State& state)
{
    const std::unique_lock<std::shared_mutex> lock(_mutex);
    // Change
    _state = state;
    ...
    switch (state)
    {
        case State::On:
            ...
            {
                if (-1 == _device->setLedDrvOut(_pin, PCA9635::LedDriver::LDRx_OFF))
                {
                    return -1;
                }
            }
            ...
            {
                if (-1 == _device->setLedDrvOut(_pin, PCA9635::LedDriver::LDRx_ON))
                {
                    return -1;
                }
            }
            ...
            {
                if (-1 == _device->setLedDrvOut(_pin, PCA9635::LedDriver::LDRx_BRI))
                {
                    return -1;
                }
            }
...
FOUNDATION_FACTORY_REGISTER(io::pwm::PWMByPCA9635,
                            "io::pwm::PWMByPCA9635",
                            io_pwm_PWMByPCA9635)
