 ...
 *
 * @file SerportTopics.hpp
 * @brief Domain topics for remote serport over ITransport.
 */
#pragma once
...
{
public:
    SerportTopics(tools::design::ipc::TopicScheme topics, std::string objectName) : _topics(std::move(topics)), _object(std::move(objectName))
    {
    }

    [[nodiscard]] const std::string& objectName() const noexcept { return _object; }

    [[nodiscard]] std::string tx() const { return channel("tx"); }
    [[nodiscard]] std::string rx() const { return channel("rx"); }
...
private:
    ...
    {
        return _topics.appTopic({"serport", _object, name});
    }
